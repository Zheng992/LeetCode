...
public class Solution {
    //法1-哈希表,其思想是判断引用指向内存地址是否一致
    // 即使值相同,内存地址不一样也可以存入.因为哈希表存放的是内存地址
    //先用一个哈希表存放ListA,再逐个判断ListB中的结点是否存在于哈希表中,若存在,代表往后都存在.(题目特性),故返回重复起始结点即可.
    /*public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        //建立一个HashSet,将链表A的所有值存入set中
